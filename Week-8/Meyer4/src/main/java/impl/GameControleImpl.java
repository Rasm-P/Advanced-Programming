        ...
        switch (answer) {
            ...
            case 2:
                ...
                try {
                    ...
                    System.out.println("Server started on " + adr + " :" + 5555);
                    TextUI.canConnectNow();
                    for (int i = 1; i < amount + 1; i++) {
                        OnlinePlayerImpl player = new OnlinePlayerImpl(server.newClientHandler());
                        player.setName(player.getEchoClientHandler().get());
                        playerList.add(player);
                        TextUI.println(player.getEchoClientHandler().put(TextUI.youAreConnected(player.getName())));
                    }
                    num = 0;
